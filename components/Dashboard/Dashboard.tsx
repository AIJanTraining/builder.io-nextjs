...
function Dashboard({ balance = "$ 1,893.44" }: DashboardProps) {
  ...
  const usersData: User[] = [
    { id: 1, name: "Alice Johnson", email: "alice.johnson@example.com", status: "Active", dateJoined: "15/01/2024", role: "Admin" },
    { id: 2, name: "Bob Smith", email: "bob.smith@example.com", status: "Active", dateJoined: "20/02/2024", role: "User" },
    { id: 3, name: "Carol White", email: "carol.white@example.com", status: "Inactive", dateJoined: "10/03/2024", role: "User" },
    { id: 4, name: "David Brown", email: "david.brown@example.com", status: "Active", dateJoined: "25/03/2024", role: "Manager" },
    { id: 5, name: "Emma Davis", email: "emma.davis@example.com", status: "Active", dateJoined: "05/04/2024", role: "User" },
  ];
    ...
    <div className={styles.dashboard}>
      ...
      <aside className={styles.sideNav}>
        <div className={styles.navItems}>
          <div className={`${styles.navItem} ${activeTab === "dashboard" ? styles.navItemActive : ""}`} onClick={() => setActiveTab("dashboard")}>
          ...
          <div className={`${styles.navItem} ${activeTab === "events" ? styles.navItemActive : ""}`} onClick={() => setActiveTab("events")}>
            <svg className={styles.navIcon} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className={`${styles.navItem} ${activeTab === "users" ? styles.navItemActive : ""}`} onClick={() => setActiveTab("users")}>
            <svg className={styles.navIcon} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <main className={styles.mainContent}>
              ...
              <tbody>
                {usersData.map((user) => (
                  <tr key={user.id} className={styles.tableRow}>
                    ...
                    <td className={styles.tableCell}>{user.dateJoined}</td>
                    <td className={`${styles.tableCell} ${user.status === "Active" ? styles.statusActive : styles.statusInactive}`}>{user.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab === "events" && (
          <div className={styles.emptyStateContainer}>
            <p className={styles.emptyStateText}>Events tab content coming soon</p>
          </div>
        )}
      </main>
    </div>
